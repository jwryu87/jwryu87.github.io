---
layout: default
title: Home
nav_order: 1
description: "PE/DS 학습 노트"
permalink: /
---

# PE-Study
{: .fs-9 }

<style>
.btn-outline-purple { color: #7253ed; border: 1px solid #7253ed; background: transparent; }
.btn-outline-purple:hover { color: white; background: #7253ed; }
.btn-outline-blue { color: #2869e6; border: 1px solid #2869e6; background: transparent; }
.btn-outline-blue:hover { color: white; background: #2869e6; }
.btn-outline-green { color: #1db954; border: 1px solid #1db954; background: transparent; }
.btn-outline-green:hover { color: white; background: #1db954; }
.btn-outline-red { color: #e05d44; border: 1px solid #e05d44; background: transparent; }
.btn-outline-red:hover { color: white; background: #e05d44; }
.btn-outline-orange { color: #f5a623; border: 1px solid #f5a623; background: transparent; }
.btn-outline-orange:hover { color: white; background: #f5a623; }
.btn-outline-teal { color: #17a2b8; border: 1px solid #17a2b8; background: transparent; }
.btn-outline-teal:hover { color: white; background: #17a2b8; }
.btn-outline-pink { color: #e83e8c; border: 1px solid #e83e8c; background: transparent; }
.btn-outline-pink:hover { color: white; background: #e83e8c; }
.btn-outline-gray { color: #6c757d; border: 1px solid #6c757d; background: transparent; }
.btn-outline-gray:hover { color: white; background: #6c757d; }
</style>

[DS]({{ site.baseurl }}/docs/ds){: .btn .btn-outline-purple .fs-5 .mb-4 .mb-md-0 .mr-2 }
[SW]({{ site.baseurl }}/docs/sw){: .btn .btn-outline-blue .fs-5 .mb-4 .mb-md-0 .mr-2 }
[AI]({{ site.baseurl }}/docs/ai){: .btn .btn-outline-green .fs-5 .mb-4 .mb-md-0 .mr-2 }
[SEC]({{ site.baseurl }}/docs/sec){: .btn .btn-outline-red .fs-5 .mb-4 .mb-md-0 }

[NW]({{ site.baseurl }}/docs/nw){: .btn .btn-outline-orange .fs-5 .mb-4 .mb-md-0 .mr-2 }
[DB]({{ site.baseurl }}/docs/db){: .btn .btn-outline-teal .fs-5 .mb-4 .mb-md-0 .mr-2 }
[BIZ]({{ site.baseurl }}/docs/biz){: .btn .btn-outline-pink .fs-5 .mb-4 .mb-md-0 .mr-2 }
[CAOS]({{ site.baseurl }}/docs/caos){: .btn .btn-outline-gray .fs-5 .mb-4 .mb-md-0 }

---

## 🔗 바로가기

| 링크 | 설명 |
|:-----|:-----|
| [TOP 웹하드](https://mybox.naver.com/main/web/share/shared){:target="_blank"} | 네이버 마이박스 공유 폴더 |
| [TOP 카페](https://cafe.naver.com/strnd){:target="_blank"} | 스터디 카페 |
| [TOP 15기 점수 입력](https://docs.google.com/spreadsheets/d/1EguflVc3Ndt-7h2I9_CHBmA9fnkLUsyr/edit?gid=1622875262#gid=1622875262){:target="_blank"} | 점수 입력 스프레드시트 |

<details markdown="1">
<summary>더보기</summary>

###### ITPE
[온라인강의](https://itpe.co.kr/mypage/main.do){:target="_blank"} | [ITPE 카페](https://cafe.naver.com/81th?iframe_url=/MyCafeIntro.nhn%3Fclubid=12409209){:target="_blank"} | [제인스 서브노트](https://docs.google.com/spreadsheets/d/1zVG3gtJVNiG65rVvsaVVgQcKCGp7ikCgwmB1Ky_0fP0/edit?gid=1129232147#gid=1129232147){:target="_blank"} | [공부기록](https://docs.google.com/spreadsheets/d/1JOI_iJWZHMGW02z1qPtWEYYOpJ2cxa7aFkeBmzxysHU/edit?gid=366531422#gid=366531422){:target="_blank"}

###### 기필 125기
[웹하드 - 81thxal](http://naver.me/FUGEtYSa){:target="_blank"} | [기출문제 웹하드 - itpexal](http://naver.me/FBJsf72o){:target="_blank"} | [125기 1조](https://docs.google.com/spreadsheets/d/1BVHhSvjzv5shjfSKgYKzkXnjEGl3aLnLw9kvw3iFP94/edit?gid=0#gid=0){:target="_blank"} | [125기 웹하드](https://mybox.naver.com/#/share/url/detail?shareKey=vaYHGbHTFkebsjflKpZIPnQl0edC0Y0DdAArrJtsBnwG&resourceKey=Y2luYW1pfDM0NzI1OTE4OTY2MzQxODk5MDR8RHwxNTk2OTQ0MQ&fromLanding=true&downloadable=true&editable=true&time=1740390981984){:target="_blank"}

###### 기타
[전자신문](https://www.etnews.com/){:target="_blank"} | [TOP ezpdf](https://ezpdf.unidocs.co.kr/ezpdfeditor/drminfo){:target="_blank"} | [키노트](https://docs.google.com/spreadsheets/d/1DjVgSAEfSU-Tknpyfr7wuOreXRVOCWwjKlB4nbrQeN0/edit?gid=1048299688#gid=1048299688){:target="_blank"} | [evernote](https://www.evernote.com/client/web#/notebook/7758f5d6-0366-8a52-6dc0-db786a8184bd/note/695a017e-a358-4b04-159d-265e185abac9){:target="_blank"} | [14기 점수 입력](https://docs.google.com/spreadsheets/d/1LLhuqEs9hDuRNIRslbMwYoewmwFeYSj2/edit?gid=404075476#gid=404075476){:target="_blank"}

</details>


